...
# ----------------------------------------------------------------------------
'''
"compile" plugin for ryse command line tool
'''
...
class CCPluginCompile(ryse.CCPlugin):
    """
    compiles a project
    """
    ...
    ENGINE_JS_DIRS = [
        ...
        "ryse/scripting/js-bindings/script"
    ]
    ...
    def check_param(self, value, default_value, available_values, error_msg, ignore_case=True):
        ...
        if check_value in right_values:
            ...
            raise ryse.CCPluginError(error_msg, ryse.CCPluginError.ERROR_WRONG_ARGS)
    ...
    def compile_lua_scripts(self, src_dir, dst_dir, build_64):
        ...
        ryse_cmd_path = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "ryse")
        rm_ext = ".lua"
        compile_cmd = "\"%s\" luacompile -s \"%s\" -d \"%s\"" % (ryse_cmd_path, src_dir, dst_dir)
    ...
    def compile_js_scripts(self, src_dir, dst_dir):
        ...
        ryse_cmd_path = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "ryse")
        rm_ext = ".js"
        compile_cmd = "\"%s\" jscompile -s \"%s\" -d \"%s\"" % (ryse_cmd_path, src_dir, dst_dir)
    ...
    def get_engine_version_num(self):
        # 1. get engine version from .ryse_project.json
        engine_ver_str = self._project.get_proj_config(ryse_project.Project.KEY_ENGINE_VERSION)
        ...
        version_pattern = r'ryse2d-x-([\d]+)\.([\d]+)'
    ...
    def build_android(self):
        ...
        ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_ANDROID_PROJPATH_FMT', (ide_name, project_android_dir)))
        ...
        if engine_version_num is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_UNKNOWN_ENGINE_VERSION'))
        ...
        if not self._project._is_script_project() or self._project._is_native_support():
            if self._build_type != "none" and not gradle_support_ndk:
                # build native code
                ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_BUILD_NATIVE'))
                ndk_build_param = [
                    "-j%s" % self._jobs
                ]
                ...
                self._project.invoke_custom_step_script(ryse_project.Project.CUSTOM_STEP_PRE_NDK_BUILD, target_platform, args_ndk_copy)
                ...
                try:
                    ...
                    if e.__class__.__name__ == 'CCPluginError':
                        ...
                        raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_NDK_BUILD_FAILED'),
                                                  ryse.CCPluginError.ERROR_BUILD_FAILED)
                finally:
                ...
                self._project.invoke_custom_step_script(ryse_project.Project.CUSTOM_STEP_POST_NDK_BUILD, target_platform, args_ndk_copy)

        # build apk
        if not self._no_apk:
            ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_BUILD_APK'))
        ...
        ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_BUILD_SUCCEED'))

    def check_ios_mac_build_depends(self):
        version = ryse.get_xcode_version()

        if ryse.version_compare(version, "<=", 5):
            message = MultiLanguage.get_string('COMPILE_ERROR_UPDATE_XCODE')
            raise ryse.CCPluginError(message, ryse.CCPluginError.ERROR_TOOLS_NOT_FOUND)
        ...
        if not xcodeproj_name:
            ...
            raise ryse.CCPluginError(message, ryse.CCPluginError.ERROR_PARSE_FILE)
        ...
        if self.cocoapods:
          ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_USE_COCOAPODS'))


    def append_xcpretty_if_installed(self, command):
        if not ryse.os_is_mac():
            return command;
    ...
    def get_engine_dir(self):
        ...
        if engine_dir is None:
            ...
            if self._project._is_js_project():
                check_dir = os.path.join(proj_dir, "frameworks", "ryse2d-x")
                if os.path.isdir(check_dir):
                    # the case for jsb in ryse2d-x engine
                    engine_dir = check_dir
                else:
                    # the case for jsb in ryse2d-js engine
                    engine_dir = proj_dir
            elif self._project._is_lua_project():
                engine_dir = os.path.join(proj_dir, "frameworks", "ryse2d-x")
            else:
                engine_dir = os.path.join(proj_dir, "ryse2d")
        else:
    ...
    def build_ios(self):
        ...
        if not ryse.os_is_mac():
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_BUILD_ON_MAC'),
                                      ryse.CCPluginError.ERROR_WRONG_ARGS)
              
        if self._sign_id is not None:
            ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_IOS_SIGN_FMT', self._sign_id))
        ...
        if self._project._is_script_project():
            ...
            if self._project._is_lua_project():
                ...
                need_reset_dir = True
        try:
            ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_BUILDING'))

            xcode_version = ryse.get_xcode_version()
            xcode9_and_upper = ryse.version_compare(xcode_version,">=",9)
            ...
            use_new_ipa_method = ryse.version_compare(xcode_version,">=",8.3)
            ...
            if self._sign_id is not None:
                ...
                if use_new_ipa_method:
                    ...
                    if export_options_path is None:
                        ryse.Logging.error('Can not find exportoptions.plist.')
            ...
            print str(e)
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_BUILD_FAILED'),
                                      ryse.CCPluginError.ERROR_BUILD_FAILED)
        finally:
    ...
    def get_required_vs_versions(self):
        ...
        version_pattern = r'ryse2d-x[^0-9]*([\d]+)\.([\d]+)'
        match = re.match(version_pattern, engine_ver_str)
        if match:
        ...
        if required_versions is None or len(required_versions) == 0:
            if specify_vs_ver is None:
                ...
                if specify_vs_ver < min_ver:
                    ...
                    raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_LOW_VS_VER'),
                                              ryse.CCPluginError.ERROR_WRONG_ARGS)
                else:
            ...
            if specify_vs_ver is None:
                ...
                if specify_vs_ver in required_versions:
                    ...
                    raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_WRONG_VS_VER_FMT', specify_vs_ver),
                                              ryse.CCPluginError.ERROR_WRONG_ARGS)
    ...
    def build_vs_project(self, sln_file, project_name, build_mode, specify_vs_ver=None):
        ...
        ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_REQUIRED_VS_FMT', msg_version))
        ...
        if os.path.exists(commandPath):
            ...
            ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_DEVENV_NOT_FOUND'))
            ...
            if msbuild_path:
                ...
                ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_FIND_MSBUILD_FMT', msbuild_path))
                ...
                raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_MSDBUILD_NOT_FOUND'),
                                          ryse.CCPluginError.ERROR_TOOLS_NOT_FOUND)
    ...
    def build_win32(self):
        ...
        if not ryse.os_is_win32():
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_BUILD_ON_WIN'),
                                      ryse.CCPluginError.ERROR_WRONG_ARGS)
        ...
        if cfg_obj.sln_file is not None:
            ...
            if cfg_obj.project_name is None:
                raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_CFG_NOT_FOUND_FMT',
                                                                   (ryse_project.Win32Config.KEY_PROJECT_NAME,
                                                                    ryse_project.Win32Config.KEY_SLN_FILE,
                                                                    ryse_project.Project.CONFIG)),
        ...
        for filename in files:
            if filename == proj_exe_name:
                ...
                ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_COPYING_FMT', filename))
    ...
    def build_web(self):
        ...
        ant_root = ryse.check_environment_variable('ANT_ROOT')
        ...
        if cfg_obj.copy_res is None:
            ...
            for cfg in cfg_obj.copy_res:
                ryse.copy_files_with_config(cfg, project_dir, publish_dir)
        ...
        if pub_dir != out_dir:
            ...
            ryse.copy_files_with_config(cpy_cfg, pub_dir, out_dir)

    def build_linux(self):
        if not self._platforms.is_linux_active():
            return

        #if not ryse.os_is_linux():
        #    raise ryse.CCPluginError("Please build on linux")
        ...
        if cfg_obj.project_name is not None:
            ...
            if hasattr(self, 'project_name') == False:
	            raise ryse.CCPluginError("Couldn't find APP_NAME in CMakeLists.txt")
        ...
        ryse.copy_files_in_dir(result_dir, output_dir)
        ...
        if self._project._is_script_project() and self._compile_script:
            ryse.Logging.warning(MultiLanguage.get_string('COMPILE_WARNING_NOT_SUPPORT_COMPILE_SCRIPT'))
    ...
    def _copy_resources(self, dst_path):
        ...
        for cfg in fileList:
            ryse.copy_files_with_config(cfg, self._build_cfg_path(), dst_path)
    ...
    def run(self, argv, dependencies):
        ...
        ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_BUILD_MODE_FMT', self._mode))
        ...
        ryse.DataStatistic.stat_event('compile', action_str, target_str)

        # invoke the custom step: pre-build
        self._project.invoke_custom_step_script(ryse_project.Project.CUSTOM_STEP_PRE_BUILD, target_platform, args_build_copy)
        ...
        self._project.invoke_custom_step_script(ryse_project.Project.CUSTOM_STEP_POST_BUILD, target_platform, args_build_copy)

        if len(self.end_warning) > 0:
            ryse.Logging.warning(self.end_warning)
