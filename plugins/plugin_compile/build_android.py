...
class AndroidBuilder(object):
    ...
    def __init__(self, verbose, app_android_root, no_res, proj_obj, mode, build_type, app_abi, gradle_support_ndk=False):
        ...
        self.sdk_root = ryse.check_environment_variable('ANDROID_SDK_ROOT')
        self.ant_root = None
    ...
    def _run_cmd(self, command, cwd=None):
        ryse.CMDRunner.run_cmd(command, self._verbose, cwd=cwd)
    ...
    def _parse_cfg(self):
        ...
        try:
            ...
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_PARSE_CFG_FAILED_FMT', self.cfg_path),
                                      ryse.CCPluginError.ERROR_PARSE_FILE)
    ...
    def _get_android_sdk_tools_ver(self, sdk_tools_path):
        ...
        raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_UNKNOWN_ANDROID_SDK_TOOLS_VERSION'),
    ...
    def _write_local_properties(self, folder_path):
        ...
        ndk_dir = ryse.check_environment_variable('NDK_ROOT')
        if ryse.os_is_win32():
    ...
    def do_ndk_build(self, ndk_build_param, mode, build_type, compile_obj):
        ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_NDK_BUILD_TYPE', build_type))
        ndk_root = ryse.check_environment_variable('NDK_ROOT')
        ...
        ndk_path = ryse.CMDRunner.convert_path_to_cmd(os.path.join(ndk_root, "ndk-build"))
        ...
        for line in open(property_file):
            ...
            if match is not None:
                ...
                if os.path.isdir(abs_lib_path):
                    ...
                    command = "%s update lib-project -p %s -t %s" % (ryse.CMDRunner.convert_path_to_cmd(sdk_tool_path), abs_lib_path, target_str)
    ...
    def get_api_level(self, target_str, raise_error=True):
        ...
        if match is not None:
            ...
            if raise_error:
                raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_NOT_VALID_AP_FMT', target_str),
                                          ryse.CCPluginError.ERROR_PARSE_FILE)
            else:
                ret = -1
    ...
    def get_target_config(self, proj_path):
        ...
        if not os.path.isfile(property_file):
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_FILE_NOT_FOUND_FMT', property_file),
                                      ryse.CCPluginError.ERROR_PATH_NOT_FOUND)
        ...
        raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_TARGET_NOT_FOUND_FMT', property_file),
                                  ryse.CCPluginError.ERROR_PARSE_FILE)
    ...
    def check_android_platform(self, sdk_root, android_platform, proj_path):
        ...
        ret_path = os.path.join(ryse.CMDRunner.convert_path_to_python(sdk_root), "platforms", ret)
        if not os.path.isdir(ret_path):
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_NO_AP_IN_SDK_FMT', ret),
                                      ryse.CCPluginError.ERROR_PATH_NOT_FOUND)
    ...
    def gradle_build_apk(self, mode, android_platform, compile_obj):
        ...
        if compile_sdk_ver is not None:
            ...
            if not os.path.isdir(check_path):
                ryse.Logging.warning(MultiLanguage.get_string('COMPILE_WARNING_COMPILE_SDK_FMT',
        ...
        if build_tools_ver is not None:
            ...
            if not os.path.isdir(check_path):
                ryse.Logging.warning(MultiLanguage.get_string('COMPILE_WARNING_BUILD_TOOLS_FMT',
        ...
        if not os.path.isfile(gradle_path):
            raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_GRALEW_NOT_EXIST_FMT', gradle_path),
                                      ryse.CCPluginError.ERROR_PATH_NOT_FOUND)
    ...
    def do_build_apk(self, mode, no_apk, no_sign, output_dir, custom_step_args, android_platform, compile_obj):
            ...
            if self._project._is_lua_project():
                ...
                if build_arch == self.LuaBuildArch.ONLY_BUILD_64BIT:
                    ...
                    if is_compiled:
                        ...
                        shutil.rmtree(os.path.join(src_dir, 'ryse'))
        ...
        if not no_apk:
            ...
            if output_dir:
                ...
                ryse.Logging.info(MultiLanguage.get_string('COMPILE_INFO_MOVE_APK_FMT', output_dir))
                ...
                raise ryse.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_NOT_SPECIFY_OUTPUT'),
    ...
    def _gather_sign_info(self):
        ...
        while True:
            ...
            if os.path.isfile(abs_path):
                ...
                ryse.Logging.warning(MultiLanguage.get_string('COMPILE_INFO_NOT_A_FILE'))
    ...
    def _copy_resources(self, custom_step_args, assets_dir):
        ...
        target_platform = ryse_project.Platforms.ANDROID
        ...
        self._project.invoke_custom_step_script(ryse_project.Project.CUSTOM_STEP_PRE_COPY_ASSETS, target_platform, cur_custom_step_args)

        # copy resources
        for cfg in res_files:
            ryse.copy_files_with_config(cfg, app_android_root, assets_dir)

        # invoke custom step : post copy assets
        self._project.invoke_custom_step_script(ryse_project.Project.CUSTOM_STEP_POST_COPY_ASSETS, target_platform, cur_custom_step_args)
