...
class TemplateGenerator(ryse.CCPlugin):
    ...
    def copy_template(self):
        for item in self.config_json[TemplateGenerator.KEY_COPY_FROM_ENGINE]:
            ryse.copy_files_with_config(item, self.engine_path, self.engine_template_dir)

        for item in self.config_json[TemplateGenerator.KEY_COPY_FROM_BIN_TEMP]:
            ryse.copy_files_with_config(item, os.path.join(self.cur_dir, 'bin-templates'), self.engine_template_dir)
    ...
    def get_version_from_source(self):
        ...
        version_file_path = os.path.join(src_engine_path, "ryse/ryse2d.cpp")
        pattern = r".*return[ \t]+\"(.*)\";"
    ...
    def gen_template_config(self, template_path, engine_ver):
        for name in os.listdir(template_path):
            ...
            cfg_path = os.path.join(fullPath, ".ryse-project.json")
            cfg_info = {}
    ...
    def modify_build_cfg(self):
        ...
        fw_version_path = "${RYSE_X_ROOT}/%s" % self.fw_version
