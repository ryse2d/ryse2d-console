...
from ryse import CCPluginError, Logging
from argparse import ArgumentParser

class SimulatorCompiler(ryse.CCPlugin):
    ...
    RYSE_CMD_PATH = 'tools/ryse2d-console/bin/ryse'
    ...
    def init(self, args):
        ...
        self.ryse_bin = os.path.join(self.engine_root, SimulatorCompiler.RYSE_CMD_PATH)
        engine_version = utils.get_engine_version(self.engine_root)
        # get the short version after "ryse2d-x-"
    ...
    def get_keywords(self):
        osx_keyword = {
            ...
            "RYSE2D_DEBUG=0":"RYSE2D_DEBUG=1",
        }

        win_keyword = {
            "_WINDOWS":"_WINDOWS;RYSE2D_DEBUG=1",
        }
    ...
    def get_depend_project_file_list(self):
        ...
        if ryse.os_is_mac() and (self.build_mac or self.build_ios):
        ...
        if ryse.os_is_win32() and self.build_android:
    ...
    def compile_for_osx(self):
        ...
        command = ' '.join([
            ...
            " && %s compile -p mac -m %s -o \"%s\" --no-res --compile-script 0" % (self.ryse_bin
    ...
    def compile_for_ios(self):
        ...
        command = ' '.join([
            " %s compile -p ios -m %s -o \"%s\" --no-res --compile-script 0" % (self.ryse_bin
                , "debug" if self.mode == 'debug' else "release"
    ...
    def compile_for_win32(self):
        ...
        if self.mode == 'debug':
            ...
            command = ' '.join([
                " %s compile -p win32 -m debug --no-res --compile-script 0 %s" % (self.ryse_bin, ver_param),
            ...
            command = " %s compile -p win32 -m release --no-res --compile-script 0 -o %s %s" % (self.ryse_bin,win32_output_dir,ver_param)
    ...
    def compile_for_android(self):
        ...
        command = ' '.join([
            " %s compile -p android --mode %s -o \"%s\" --no-res --compile-script 0" % (self.ryse_bin
                 , "debug" if self.mode == 'debug' else "release"
    ...
    def change_ryse2d_debug_macro_to_1(self, file_list):
    ...
    def update_bundle_version(self):
        ...
        if ryse.os_is_mac() and self.build_mac:
            # mac
        ...
        if ryse.os_is_win32() and self.build_win:
            # win32
    ...
    def run(self, argv, dependencies):
        ...
        try:
            ...
            self.change_ryse2d_debug_macro_to_1(modify_files)
