...
XCODE_LINK_LUA_LIBS = [ "libluaryse2d", "libsimulator" ]

XCODE_LINK_JS_LIBS = [ "libjsryse2d", "libsimulator" ]
...
class TemplateModifier(object):
    ...
    def modify_xcode_proj(self, proj_file_path):
        ...
        if language == "cpp":
            ...
            replace_engine_strs.append("$(SRCROOT)/../ryse2d")
        elif language == "lua":
            targetName = "HelloLua"
            link_libs = XCODE_LINK_CPP_LIBS + XCODE_LINK_LUA_LIBS
            replace_engine_strs.append("$(SRCROOT)/../../ryse2d-x")
        else:
            targetName = "HelloJavascript"
            link_libs = XCODE_LINK_CPP_LIBS + XCODE_LINK_JS_LIBS
            replace_engine_strs.append("$(SRCROOT)/../../ryse2d-x")
            replace_engine_strs.append("../../ryse2d-x")
        ios_target_name = "%s-mobile" % targetName
        mac_target_name = "%s-desktop" % targetName

        # remove the target dependencies
        pbx_proj.remove_proj_reference("ryse2d_libs.xcodeproj")
        if language == "lua":
            pbx_proj.remove_proj_reference("ryse2d_lua_bindings.xcodeproj")
            pbx_proj.remove_proj_reference("libsimulator.xcodeproj")

        if language == "js":
            pbx_proj.remove_proj_reference("ryse2d_js_bindings.xcodeproj")
            pbx_proj.remove_proj_reference("libsimulator.xcodeproj")
            pbx_proj.remove_file_by_path("../../ryse2d-x/ryse/scripting/js-bindings/script")
        ...
        libs_path = "/Applications/Ryse/Ryse2d-x/%s/prebuilt" % self.version
        ...
        install_path = "/Applications/Ryse/Ryse2d-x/%s" % self.version
    ...
    def modify_vs_proj(self, proj_file_path):
        ...
        install_path = "$(RYSE_X_ROOT)\\%s\\" % self.version
        ...
        if language == "js":
            ...
            custom_step_event.replace("$(ProjectDir)..\\..\\ryse2d-x\\ryse\\scripting\\js-bindings\\script",
                                      "$(ProjectDir)..\\..\\..\\script")
        ...
        if language == "cpp":
            ...
            replace_strs.append("$(ProjectDir)..\\ryse2d")
            replace_strs.append("..\\ryse2d")
        elif language == "lua":
            # link_libs = WIN32_LINK_CPP_LIBS + WIN32_LINK_LUA_LIBS
            replace_strs.append("$(ProjectDir)..\\..\\ryse2d-x")
            replace_strs.append("..\\..\\ryse2d-x")
        else:
        ...
        if os.path.exists(cpp_path):
            ...
            file_content = file_content.replace("#ifndef _DEBUG", "#ifndef RYSE2D_DEBUG")
            f = open(cpp_path, "w")
            f.write(file_content)
            f.close()
        ...
        file_content = file_content.replace("%sryse\\2d\\ryse2dx.props" % install_path, "ryse2dx.props")
