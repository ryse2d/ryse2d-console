...
# ----------------------------------------------------------------------------
'''
"run" plugin for ryse command line tool
'''
...
class CCPluginRun(ryse.CCPlugin):
    """
    ...
    def _get_simulator_id(self):
            ...
            for info in names:
                ...
                if match:
                    ...
                    if ((typeNum > phoneTypeNum) or
                        ...
                        (typeNum == phoneTypeNum and tmpType == phoneType and ryse.version_compare(tmpIOSVer, '>', iosVer))):
        ...
        if ret is None:
            raise ryse.CCPluginError('Get simulator failed!')
    ...
    def _get_bundle_id(self, app_path):
        ...
        if ret is None:
            raise ryse.CCPluginError('Get the bundle ID of app %s failed' % app_path)

        return ret

    def _run_ios_app(self, ios_app_path):
        if not ryse.os_is_mac():
            raise ryse.CCPluginError('Now only support run iOS simulator on Mac OS')
    ...
    def run_ios_sim(self, dependencies):
        ...
        if deploy_dep._use_sdk == 'iphoneos':
            ryse.Logging.warning(MultiLanguage.get_string('RUN_WARNING_IOS_FOR_DEVICE_FMT',
    ...
    def run_android_device(self, dependencies):
        ...
        sdk_root = ryse.check_environment_variable('ANDROID_SDK_ROOT')
        adb_path = ryse.CMDRunner.convert_path_to_cmd(os.path.join(sdk_root, 'platform-tools', 'adb'))
        deploy_dep = dependencies['deploy']
    ...
    def run_web(self, dependencies):
        ...
        while (i <= port_max_add):
            ...
            try:
                ryse.Logging.info(MultiLanguage.get_string('RUN_INFO_HOST_PORT_FMT', (host, port)))
                httpd = ServerClass(server_address, HandlerClass)
            except Exception as e:
                httpd = None
                ryse.Logging.warning(MultiLanguage.get_string('RUN_WARNING_SERVER_FAILED_FMT', (host, port, e)))

            if httpd is not None:
                break

        if httpd is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('RUN_ERROR_START_SERVER_FAILED'),
                                      ryse.CCPluginError.ERROR_OTHERS)
        ...
        with ryse.pushd(run_root):
            ryse.Logging.info(MultiLanguage.get_string('RUN_INFO_SERVING_FMT', (sa[0], sa[1])))
            httpd.serve_forever()
    ...
    def run_tizen(self, dependencies):
        ...
        tizen_studio_path = ryse.check_environment_variable("TIZEN_STUDIO_HOME")
        tizen_cmd_path = ryse.CMDRunner.convert_path_to_cmd(os.path.join(tizen_studio_path, "tools", "ide", "bin", "tizen"))
    ...
    def run(self, argv, dependencies):
        ...
        ryse.Logging.info(MultiLanguage.get_string('RUN_INFO_START_APP'))
