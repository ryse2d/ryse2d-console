...
# ryse "new" plugin
#
# Copyright 2013 (C) ryse2d-x.org
#
# License: MIT
# ----------------------------------------------------------------------------
'''
"new" plugin for ryse command line tool
'''
...
#
class CCPluginNew(ryse.CCPlugin):

    DEFAULT_PROJ_NAME = {
        ryse_project.Project.CPP: 'MyCppGame',
        ryse_project.Project.LUA: 'MyLuaGame',
        ryse_project.Project.JS: 'MyJSGame'
    }
    ...
    def init(self, args):
        ...
        # new official ways to get the template and ryse paths
        self._templates_paths = self.get_templates_paths()
        self._ryseroot = self.get_ryse2d_path()

        # search for custom paths
        if args.engine_path is not None:
            self._ryseroot = os.path.abspath(args.engine_path)
            self._ryseroot = unicode(self._ryseroot, "utf-8")
            tp_path = os.path.join(self._ryseroot, "templates")
    ...
    def parse_args(self, argv):
        ...
        parser = ArgumentParser(prog="ryse %s" % self.__class__.plugin_name(),
        ...
        # REMOVE the option --no-native. Because it's added for Ryse Code IDE.
        # It will cause confusion: https://github.com/ryse2d/ryse2d-console/issues/401
...
#            sys.exit(0)
        ...
        if not args.package:
            args.package = "org.ryse2dx.%s" % args.name
    ...
    def _stat_engine_version(self):
        try:
            ...
            framework_ver_file = os.path.join(self._ryseroot, 'version')
            x_ver_file = os.path.join(self._ryseroot, 'ryse/ryse2d.cpp')
            js_ver_file = os.path.join(self._ryseroot, 'frameworks/js-bindings/bindings/manual/ScriptingCore.h')
            if os.path.isfile(framework_ver_file):
                # the engine is Ryse Framework
                f = open(framework_ver_file)
                ver_str = f.read()
                f.close()
                engine_type = 'ryseframework'
            else:
                ver_file = None
                pattern = None
                if os.path.isfile(x_ver_file):
                    # the engine is ryse2d-x
                    pattern = r".*return[ \t]+\"(.*)\";"
                    ver_file = x_ver_file
                    engine_type = 'ryse2d-x'
                elif os.path.isfile(js_ver_file):
                    # the engine is ryse2d-js
                    pattern = r".*#define[ \t]+ENGINE_VERSION[ \t]+\"(.*)\""
                    ver_file = js_ver_file
                    engine_type = 'ryse2d-js'
            ...
            if ver_str is not None:
                ...
                ryse.DataStatistic.stat_event('new_engine_ver', ver_str, engine_type)
        except:
            pass
    ...
    def _create_from_cmd(self):
        ...
        if os.path.exists(self._projdir):
            ...
            raise ryse.CCPluginError(message, ryse.CCPluginError.ERROR_PATH_NOT_FOUND)

        creator = TPCreator(self._lang, self._ryseroot, self._projname, self._projdir,
        ...
        data = None
        cfg_path = os.path.join(self._projdir, ryse_project.Project.CONFIG)
        ...
        if ryse_project.Project.KEY_PROJ_TYPE not in data:
            data[ryse_project.Project.KEY_PROJ_TYPE] = self._lang

        # script project may add native support
        if self._lang in (ryse_project.Project.LUA, ryse_project.Project.JS):
            # REMOVE the option --no-native. Because it's added for Ryse Code IDE.
            # It will cause confusion: https://github.com/ryse2d/ryse2d-console/issues/401
            # if self._other_opts.no_native is not self._other_opts.no_native:
            #     creator.do_other_step('do_add_native_support')
            #     data[ryse_project.Project.KEY_HAS_NATIVE] = True
            # else:
            #     data[ryse_project.Project.KEY_HAS_NATIVE] = False
            creator.do_other_step('do_add_native_support')
            data[ryse_project.Project.KEY_HAS_NATIVE] = True

        # record the engine version if not predefined
        if not data.has_key(ryse_project.Project.KEY_ENGINE_VERSION):
            engine_version = utils.get_engine_version(self._ryseroot)
            if engine_version is not None:
                data[ryse_project.Project.KEY_ENGINE_VERSION] = engine_version
    ...
    def run(self, argv, dependencies):
        ...
        ryse.DataStatistic.stat_event('new', action_str, self._tpname)
...
class Templates(object):
    ...
    def __init__(self, lang, templates_paths, current):
        ...
        if current is not None:
            if current in self._template_folders:
                ...
                ryse.Logging.warning(MultiLanguage.get_string('NEW_TEMPLATE_NOT_FOUND_FMT', current))
    ...
    def _scan(self):
        ...
        if len(self._template_folders) == 0:
            cur_engine = "ryse2d-x" if self._lang == "js" else "ryse2d-js"
            need_engine = "ryse2d-js" if self._lang == "js" else "ryse2d-x"
    ...
    def select_one(self):
        ryse.Logging.warning(MultiLanguage.get_string('NEW_SELECT_TEMPLATE_TIP1'))

        p = self._template_folders.keys()
        for i in range(len(p)):
            ryse.Logging.warning('%d %s' % (i + 1, p[i]))
        ryse.Logging.warning(MultiLanguage.get_string('NEW_SELECT_TEMPLATE_TIP2'))
        while True:
            option = raw_input()
            if option.isdigit():
                option = int(option) - 1
...
class TPCreator(object):
    ...
    def __init__(self, lang, ryse_root, project_name, project_dir, tp_name, tp_dir, project_package, mac_id, ios_id):
        self.lang = lang
        self.ryse_root = ryse_root
        ...
        self.tp_dir = tp_dir
        self.tp_json = 'ryse-project-template.json'
        ...
        if 'do_default' not in tpinfo:
            ...
            raise ryse.CCPluginError(message, ryse.CCPluginError.ERROR_WRONG_CONFIG)
    ...
    def cp_self(self, project_dir, exclude_files):
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_COPY_TEMPLATE_FMT', project_dir))
        ...
        ryse.copy_files_with_config(copy_cfg, self.tp_dir, self.project_dir)
    ...
    def do_other_step(self, step, not_existed_error=True):
        if step not in self.tp_other_step:
            if not_existed_error:
                ...
                # handle as warning
                ryse.Logging.warning(MultiLanguage.get_string('NEW_WARNING_STEP_NOT_FOUND_FMT', step))
                return
    ...
    def do_cmds(self, cmds):
        for k, v in cmds.iteritems():
            ...
            try:
                ...
                raise ryse.CCPluginError(MultiLanguage.get_string('NEW_ERROR_CMD_NOT_FOUND_FMT', k),
                                          ryse.CCPluginError.ERROR_WRONG_CONFIG)

            try:
                cmd(v)
            except Exception as e:
                raise ryse.CCPluginError(str(e), ryse.CCPluginError.ERROR_RUNNING_CMD)

# cmd methods below
    def append_h5_engine(self, v):
        src = os.path.join(self.ryse_root, v['from'])
        dst = os.path.join(self.project_dir, v['to'])
        # check ryse engine exist
        ...
        # begin copy engine
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_COPY_H5'))
        for index in range(len(file_list)):
            srcfile = os.path.join(src, file_list[index])
            dstfile = os.path.join(dst, file_list[index])

            srcfile = ryse.add_path_prefix(srcfile)
            dstfile = ryse.add_path_prefix(dstfile)

            if not os.path.exists(os.path.dirname(dstfile)):
                os.makedirs(ryse.add_path_prefix(os.path.dirname(dstfile)))
    ...
    def append_x_engine(self, v):
        # FIXME this is a hack, but in order to fix it correctly the ryse-project-template.json
        ...
        if ryse.CCPlugin.get_ryse2d_mode() == 'distro':
            return
        ...
        # check ryse engine exist
        rysex_files_json = os.path.join(
            src, 'templates', 'ryse2dx_files.json')
        if not os.path.exists(rysex_files_json):
            message = MultiLanguage.get_string('NEW_WARNING_FILE_NOT_FOUND_FMT', rysex_files_json)
            raise ryse.CCPluginError(message, ryse.CCPluginError.ERROR_PATH_NOT_FOUND)

        f = open(rysex_files_json)
        data = json.load(f)
        f.close()
        ...
        # begin copy engine
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_COPY_X'))
    ...
    def append_from_template(self, v):
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_APPEND_TEMPLATE'))
        ryse.copy_files_with_config(v, self.tp_dir, self.project_dir)

    def append_dir(self, v):
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_APPEND_DIR'))
        for item in v:
            ryse.copy_files_with_config(
                item, self.ryse_root, self.project_dir)

    def append_file(self, v):
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_APPEND_FILE'))
        for item in v:
            src = os.path.join(self.ryse_root, item['from'])
            dst = os.path.join(self.project_dir, item['to'])

            src = ryse.add_path_prefix(src)
            dst = ryse.add_path_prefix(dst)
...
# project cmd
    def project_rename(self, v):
        ...
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_RENAME_PROJ_FMT',
        ...
        for f in files:
            ...
            if os.path.exists(src_file_path):
                ...
                ryse.Logging.warning(MultiLanguage.get_string('NEW_WARNING_FILE_NOT_FOUND_FMT',
    ...
    def project_replace_project_name(self, v):
        ...
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_REPLACE_PROJ_FMT',
                                                    (src_project_name, dst_project_name)))
        files = v['files']
        for f in files:
            dst = f.replace("PROJECT_NAME", dst_project_name)
    ...
    def project_replace_package_name(self, v):
        ...
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_REPLACE_PKG_FMT',
                                                    (src_package_name, dst_package_name)))
        files = v['files']
        if not dst_package_name:
            raise ryse.CCPluginError(MultiLanguage.get_string('NEW_ERROR_PKG_NAME_NOT_SPECIFIED'),
                                      ryse.CCPluginError.ERROR_WRONG_ARGS)
        for f in files:
            dst = f.replace("PROJECT_NAME", dst_project_name)
            if os.path.exists(os.path.join(dst_project_dir, dst)):
                replace_string(
    ...
    def project_replace_mac_bundleid(self, v):
        ...
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_MAC_BUNDLEID_FMT',
    ...
    def project_replace_ios_bundleid(self, v):
        ...
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_IOS_BUNDLEID_FMT',
                                                    (src_bundleid, dst_bundleid)))
        files = v['files']
        for f in files:
            dst = f.replace("PROJECT_NAME", dst_project_name)
    ...
    def modify_files(self, v):
        ...
        """
        ryse.Logging.info(MultiLanguage.get_string('NEW_INFO_STEP_MODIFY_FILE'))
        for modify_info in v:
            ...
            if not os.path.isfile(modify_file):
                ryse.Logging.warning(MultiLanguage.get_string('NEW_WARNING_NOT_A_FILE_FMT', modify_file))
                continue
