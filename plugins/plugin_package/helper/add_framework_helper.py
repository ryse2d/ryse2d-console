...
class AddFrameworkHelper(object):
    ...
    MAC_PBXFRAMEWORKBUILDPHASE_TAG = '\S+ /\* libryse2d Mac.a in Frameworks \*/,'
    IOS_PBXFRAMEWORKBUILDPHASE_TAG = '\S+ /\* libryse2d iOS.a in Frameworks \*/,'

    IOS_HEADER_MATCH_TAG = '(\$\(_RYSE_HEADER_IOS_BEGIN\))(.+)(\$\(_RYSE_HEADER_IOS_END\))'
    IOS_LIB_BEGIN_TAG = '\$\(_RYSE_LIB_IOS_BEGIN\)'
    IOS_LIB_END_TAG = '\$\(_RYSE_LIB_IOS_END\)'

    MAC_HEADER_MATCH_TAG = '(\$\(_RYSE_HEADER_MAC_BEGIN\))(.+)(\$\(_RYSE_HEADER_MAC_END\))'
    MAC_LIB_BEGIN_TAG = '\$\(_RYSE_LIB_MAC_BEGIN\)'
    MAC_LIB_END_TAG = '\$\(_RYSE_LIB_MAC_END\)'

    WIN32_HEADER_MATCH_TAG = '(\$\(_RYSE_HEADER_WIN32_BEGIN\))(.+)(\$\(_RYSE_HEADER_WIN32_END\))'
    WIN32_LIB_PATH_MATCH_TAG = '(\$\(_RYSE_LIB_PATH_WIN32_BEGIN\))(.+)(\$\(_RYSE_LIB_PATH_WIN32_END\))'
    WIN32_LIB_MATCH_TAG = '(\$\(_RYSE_LIB_WIN32_BEGIN\))(.+)(\$\(_RYSE_LIB_WIN32_END\))'

    ANDROID_HEADER_BEGIN_TAG = '# _RYSE_HEADER_ANDROID_BEGIN'
    ANDROID_HEADER_END_TAG = '# _RYSE_HEADER_ANDROID_END'
    ANDROID_HEADER_PREFIX = 'LOCAL_C_INCLUDES'
    ANDROID_LIB_BEGIN_TAG = '# _RYSE_LIB_ANDROID_BEGIN'
    ANDROID_LIB_END_TAG = '# _RYSE_LIB_ANDROID_END'
    ANDROID_LIB_PREFIX = 'LOCAL_STATIC_LIBRARIES'
    ANDROID_LIB_IMPORT_BEGIN_TAG = '# _RYSE_LIB_IMPORT_ANDROID_BEGIN'
    ANDROID_LIB_IMPORT_END_TAG = '# _RYSE_LIB_IMPORT_ANDROID_END'
    ...
    def run(self):
        for command in self._commands:
            try:
                ...
                raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_CMD_NOT_FOUND_FMT', name),
                                          ryse.CCPluginError.ERROR_CMD_NOT_FOUND)

            try:
                cmd(command)
            except Exception as e:
                raise ryse.CCPluginError(str(e), ryse.CCPluginError.ERROR_OTHERS)
    ...
    def add_header_path_on_ios_mac(self, source, platform):
        if platform == "ios":
            ...
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_ERROR_INVALID_PLATFORM_FMT', platform),
        ...
        if tag_found == False:
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_TAG_NOT_FOUND_FMT',
                                      ("header", platform)),
    ...
    def add_entry_function(self, command):
        ...
        if match is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_ENTRY_DECLARE_FAILED'),
                                      ryse.CCPluginError.ERROR_PARSE_FILE)
        else:
        ...
        if match is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_ERROR_IN_FILE_FMT', file_path),
                                      ryse.CCPluginError.ERROR_PARSE_FILE)
        else:
            # add entry funtion
    ...
    def do_add_project_on_win(self, command):
        ...
        if all_text is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_ERROR_READ_SLN'),
                                      ryse.CCPluginError.ERROR_PARSE_FILE)
        ...
        if all_text is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_WIN_PROJ_NOT_FOUND'),
                                      ryse.CCPluginError.ERROR_PARSE_FILE)
    ...
    def do_add_project_on_android(self, command):
        ...
        if build_cfg_file is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_BUILD_CFG_NOT_FOUND'),
                                      ryse.CCPluginError.ERROR_PATH_NOT_FOUND)
        f = open(build_cfg_file, "rb")
        configs = json.load(f)
        f.close()
        if not isinstance(configs["ndk_module_path"], list):
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_NDK_MODULE_NOT_FOUND'),
    ...
    def load_proj_ios_mac(self, notSplitLines = False):
        ...
        if not os.path.isdir(workdir + os.sep + proj_dir):
            raise ryse.CCPluginError(MultiLanguage.get_string('PACKAGE_NOT_XCODE_PROJ_FMT', proj_dir),
                                      ryse.CCPluginError.ERROR_PATH_NOT_FOUND)
