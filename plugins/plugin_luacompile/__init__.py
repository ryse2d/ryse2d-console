...
'''
"luacompile" plugin for ryse command line tool
'''
...
#import ryse
class CCPluginLuaCompile(ryse.CCPlugin):
    """
    ...
    def init(self, options, workingdir):
        ...
        if self._luajit_exe_path is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('LUACOMPILE_ERROR_TOOL_NOT_FOUND'),
    ...
    def get_relative_path(self, luafile):
        try:
            ...
            if pos != 0:
                raise ryse.CCPluginError(MultiLanguage.get_string('LUACOMPILE_ERROR_SRCDIR_NAME_NOT_FOUND'),
    ...
    def get_output_file_path(self, luafile):
        ...
        try:
            ...
            if os.path.exists(dst_rootpath) == False:
                ...
                raise ryse.CCPluginError(MultiLanguage.get_string('LUACOMPILE_ERROR_MKDIR_FAILED_FMT',
                                                                   dst_rootpath),
    ...
    def compile_lua(self, lua_file, output_file):
        ...
        Compiles lua file
        """
        ryse.Logging.debug(MultiLanguage.get_string('LUACOMPILE_DEBUG_COMPILE_FILE_FMT', lua_file))

        with ryse.pushd(self._luajit_dir):
    ...
    def handle_all_lua_files(self):
        ...
        ryse.Logging.info(MultiLanguage.get_string('LUACOMPILE_INFO_PROCESS_FILE'))
        index = 0
    ...
    def run(self, argv, dependencies):
        ...
        # tips
        ryse.Logging.warning(MultiLanguage.get_string('LUACOMPILE_WARNING_TIP_MSG'))
        ...
        ryse.Logging.info(MultiLanguage.get_string('LUACOMPILE_INFO_FINISHED'))
    ...
    def parse_args(self, argv):
        ...
        if options.src_dir_arr == None:
            raise ryse.CCPluginError(MultiLanguage.get_string('LUACOMPILE_ERROR_SRC_NOT_SPECIFIED'),
                                      ryse.CCPluginError.ERROR_WRONG_ARGS)
        elif options.dst_dir == None:
            raise ryse.CCPluginError(MultiLanguage.get_string('LUACOMPILE_ERROR_DST_NOT_SPECIFIED'),
                                      ryse.CCPluginError.ERROR_WRONG_ARGS)
        else:
            for src_dir in options.src_dir_arr:
                if os.path.exists(src_dir) == False:
                    raise ryse.CCPluginError(MultiLanguage.get_string('LUACOMPILE_ERROR_DIR_NOT_EXISTED_FMT')
                                              % (src_dir), ryse.CCPluginError.ERROR_PATH_NOT_FOUND)
