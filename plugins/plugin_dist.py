...
# ----------------------------------------------------------------------------
'''
"dist" plugin for ryse2d command line tool
'''
...
class CCPluginDist(ryse2d.CCPlugin):
    """
    ...
    def init(self, options, working_dir):
        ...
        if self._platforms.is_ios_active():
            if not options.provisioning:
                raise ryse2d.CCPluginError("Provisioning profile is needed")
            else:
    ...
    def dist_android(self):
        ...
        raise ryse2d.CCPluginError("unimplemented")
    ...
    def _find_ios_scheme(self, project_dir):
        ...
        if match is None:
            raise ryse2d.CCPluginError("Couldn't find the schemes list")

        schemes = match.group(1).split()
        if len(schemes) == 0:
            raise ryse2d.CCPluginError("Couldn't find a scheme")
