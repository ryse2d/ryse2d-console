...
class Project(object):
    ...
    def _parse_project_json(self, src_dir):
        ...
        if proj_path == None:
            raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_CFG_NOT_FOUND_FMT',
        ...
        try:
            ...
            if f is not None:
                f.close()
            raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_CFG_BROKEN_FMT',
                                      project_json),
                                      ryse.CCPluginError.ERROR_PARSE_FILE)

        if project_info is None:
            raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_CFG_PARSE_FAILED_FMT',
                                      Project.CONFIG), ryse.CCPluginError.ERROR_PARSE_FILE)

        if not project_info.has_key(Project.KEY_PROJ_TYPE):
            raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_CFG_GET_VALUE_FAILED_FMT',
        ...
        if not (lang in Project.language_list()):
            raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_CFG_INVALID_LANG_FMT',
        ...
        if (project_info.has_key(Project.KEY_CUSTOM_STEP_SCRIPT)):
            ...
            if os.path.isfile(script_path):
                ...
                ryse.Logging.info(MultiLanguage.get_string('PROJECT_INFO_FOUND_CUSTOM_STEP_FMT', script_path))
            else:
                ryse.Logging.warning(MultiLanguage.get_string('PROJECT_WARNING_CUSTOM_SCRIPT_NOT_FOUND_FMT',
                                      script_path))
                self._custom_step = None
    ...
    def invoke_custom_step_script(self, event, tp, args):
        try:
            ...
            ryse.Logging.warning(MultiLanguage.get_string('PROJECT_WARNING_CUSTOM_STEP_FAILED_FMT', e))
            raise e
...
class Platforms(object):
    ...
    CFG_CLASS_MAP = {
        ANDROID : "ryse_project.AndroidConfig",
        IOS : "ryse_project.iOSConfig",
        MAC : "ryse_project.MacConfig",
        WEB : "ryse_project.WebConfig",
        WIN32 : "ryse_project.Win32Config",
        LINUX : "ryse_project.LinuxConfig",
        METRO : "ryse_project.MetroConfig",
        TIZEN : "ryse_project.TizenConfig"
    }
    ...
    def __init__(self, project, current, proj_dir = None):
        ...
        if current is not None:
            ...
            if current_lower in self._available_platforms.keys():
                ...
                raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_INVALID_PLATFORM_FMT',
    ...
    def _gen_available_platforms(self, proj_info, proj_dir):
        ...
        for p in platform_list:
            cfg_class = ryse.get_class(Platforms.CFG_CLASS_MAP[p])
        ...
        if len(self._available_platforms) == 0:
            raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_NO_AVAILABLE_PLATFORMS'),
                                      ryse.CCPluginError.ERROR_WRONG_CONFIG)
    ...
    def select_one(self):
        ...
        raise ryse.CCPluginError(MultiLanguage.get_string('PROJECT_SPECIFY_PLATFORM_FMT',
