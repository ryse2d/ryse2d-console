#!/usr/bin/python
# ----------------------------------------------------------------------------
# ryse-console: command line tool manager for ryse2d-x
#
...
# ----------------------------------------------------------------------------
'''
Command line tool manager for ryse
'''
...
RYSE2D_CONSOLE_VERSION = '2.3'


class Ryse2dIniParser:
    def __init__(self):
        import ConfigParser
        ...
        self.ryse2d_path = os.path.dirname(os.path.abspath(sys.argv[0]))
        self._cp.read(os.path.join(self.ryse2d_path, "ryse2d.ini"))

        # XXX: override with local config ??? why ???
        self._cp.read("~/.ryse2d-js/ryse2d.ini")
        ...
        for s in self._cp.sections():
            if s == 'plugins':
                for classname in self._cp.options(s):
                    ...
                    if name is None:
                        print(MultiLanguage.get_string('RYSE_PARSE_PLUGIN_WARNING_FMT', classname))
                    if len(category) == 0:
                        key = name
                    else:
    ...
    def _sanitize_path(self, path):
        ...
        path = os.path.abspath(os.path.join(self.ryse2d_path, path))
        if not os.path.isdir(path):
            Logging.warning(MultiLanguage.get_string('RYSE_WARNING_INVALID_DIR_IN_INI_FMT', path))
            return None
        return path
    ...
    def get_ryse2dx_path(self):
        ryse2d_x = self._cp.get('paths', 'ryse2d_x')
        ryse2d_x = self._sanitize_path(ryse2d_x)
        return ryse2d_x
    ...
    def get_ryse2dx_mode(self):
        mode = self._cp.get('global', 'ryse2d_x_mode')
        if mode is None or len(mode) == 0:
            mode = 'source'

        if mode not in ('source', 'precompiled', 'distro'):
            Logging.warning(MultiLanguage.get_string('RYSE_WARNING_INVALID_MODE_FMT', mode))
            mode = 'source'
...
class CMDRunner(object):
    ...
    def run_cmd(command, verbose, cwd=None):
        if verbose:
            Logging.debug(MultiLanguage.get_string('RYSE_DEBUG_RUNNING_CMD_FMT', ''.join(command)))
        else:
        ...
        if ret != 0:
            ...
            if not verbose:
                message += (MultiLanguage.get_string('RYSE_ERROR_CHECK_LOG_FMT', log_path))
            raise CCPluginError(message, CCPluginError.ERROR_RUNNING_CMD)

    @staticmethod
    def output_for(command, verbose):
        if verbose:
            Logging.debug(MultiLanguage.get_string('RYSE_DEBUG_RUNNING_CMD_FMT', command))
        else:
            log_path = CCPlugin._log_path()
        ...
        try:
            ...
            output = e.output
            message = MultiLanguage.get_string('RYSE_ERROR_RUNNING_CMD')
...
class DataStatistic(object):
    '''
    In order to improve ryse, we periodically send anonymous data about how you use ryse.
    You can turn off this function by change the value of "enable_stat" in ryse2d.ini.

    Information collected will be used to develop new features and improve ryse.
    ...
    def get_cfg_file_path(cls):
        return os.path.join(os.path.expanduser('~/.ryse'), 'local_cfg.json')
    ...
    def change_agree_stat(cls, agreed):
        ...
        ini_file = os.path.join(get_current_path(), "ryse2d.ini")
        f = open(ini_file)
    ...
    def show_stat_agreement(cls, skip_agree_value=None):
        ...
        if skip_agree_value is None:
            # show the agreement
            input_value = raw_input(MultiLanguage.get_string('RYSE_AGREEMENT'))
    ...
    def init_stat_obj(cls):
        if cls.inited == False:
            # get the ryse_stat module
            m = None
            try:
                m = __import__("ryse_stat")
            except:
                pass
            ...
            if cls.stat_obj is not None:
                # check config in ryse2d.ini
                parser = Ryse2dIniParser()
...
class CCPlugin(object):
    ...
    @classmethod
    def get_ryse2d_path(cls):
        """returns the path where Ryse2d-x is installed"""

        #
        # 1: Check for config.ini
        #
        parser = Ryse2dIniParser()
        ryse2dx_path = parser.get_ryse2dx_path()

        if ryse2dx_path is not None:
            return ryse2dx_path
        ...
        # /Users/myself/ryse2d-x/tools/ryse2d-console/bin
        # if so, we have to remove the last 3 segments
        path = cls.get_console_path()
        path = os.path.abspath(path)
        ryse2dx_path = os.path.abspath(os.path.join(
            path, os.path.pardir, os.path.pardir, os.path.pardir))
        if os.path.isdir(ryse2dx_path):
            return ryse2dx_path

        if cls.get_ryse2d_mode() is not "distro":
            # In 'distro' mode this is not a warning since
            # the source code is not expected to be installed
            Logging.warning(MultiLanguage.get_string('RYSE_WARNING_ENGINE_NOT_FOUND'))
        return None

    @classmethod
    def get_console_path(cls):
        """returns the path where ryse console is installed"""
    ...
    def get_templates_paths(cls):
        ...
        #
        # 2: Path defined by walking the ryse2d path
        #
        path = cls.get_ryse2d_path()

        if path is not None:
            # Try one: ryse2d-x/templates (assuming it is using ryse2d-x's setup.py)
            # Try two: ryse2d-x/../../templates
            ...
            for p in possible_paths:
                ...
                try:
                    ...
                    Logging.info(MultiLanguage.get_string('RYSE_INFO_CHECK_TEMPLATE_PATH_FAILED_FMT', template_path))
                    Logging.info("%s" % e)
                    pass

        #
        # 3: Templates can be in ~/.ryse2d/templates as well
        #
        user_path = os.path.expanduser("~/.ryse/templates")
        if os.path.isdir(user_path):
            paths.append(user_path)

        if len(paths) == 0:
            raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_TEMPLATE_NOT_FOUND'),
    ...
    @classmethod
    def get_ryse2d_mode(cls):
        parser = Ryse2dIniParser()
        return parser.get_ryse2dx_mode()

    @staticmethod
    def _log_path():
        log_dir = os.path.expanduser("~/.ryse")
        if not os.path.exists(log_dir):
            os.mkdir(log_dir)
        return os.path.join(log_dir, "ryse.log")
    ...
    def init(self, args):
        ...
        self._platforms = ryse_project.Platforms(self._project, args.platform, args.proj_dir)
    ...
    def parse_args(self, argv):
        ...
        parser.add_argument("-s", "--src",
                            dest="src_dir",
                            help=MultiLanguage.get_string('RYSE_HELP_ARG_SRC'))
        parser.add_argument("-q", "--quiet",
                            action="store_true",
                            dest="quiet",
                            help=MultiLanguage.get_string('RYSE_HELP_ARG_QUIET'))
        platform_list = ryse_project.Platforms.list_for_display()
        parser.add_argument("-p", "--platform",
                            dest="platform",
                            help=MultiLanguage.get_string('RYSE_HELP_ARG_PLATFORM'))
        ...
        parser.add_argument("--proj-dir",
                            dest="proj_dir",
                            help=MultiLanguage.get_string('RYSE_HELP_ARG_PROJ_DIR'))
        self._add_custom_options(parser)

        (args, unkonw) = parser.parse_known_args(argv)

        if args.src_dir is None:
            self._project = ryse_project.Project(os.path.abspath(os.getcwd()))
        else:
            self._project = ryse_project.Project(
                os.path.abspath(args.src_dir))

        args.src_dir = self._project.get_project_dir()
        if args.src_dir is None:
            raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_PROJECT_NOT_FOUND'),
                                CCPluginError.ERROR_WRONG_ARGS)

        if args.platform:
            args.platform = args.platform.lower()
            if args.platform not in platform_list:
                raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_UNKNOWN_PLATFORM_FMT', args.platform),
                                    CCPluginError.ERROR_WRONG_ARGS)

        if args.listplatforms and self._project is not None:
            platforms = ryse_project.Platforms(self._project, args.platform, args.proj_dir)
...
def _check_dependencies_exist(dependencies, classes, plugin_name):
    for dep in dependencies:
        if dep not in classes:
            raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_INVALID_DEPENDENCY_FMT',
                                (plugin_name, dep)),
...
def check_environment_variable(var):
    ...
    try:
        ...
        raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_ENV_NOT_DEFINED_FMT', var),
...
def get_xcode_version():
    ...
    if xcode is None:
        raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_XCODE_NOT_INSTALLED'),
...
def help():
    print(MultiLanguage.get_string('RYSE_HELP_BRIEF_FMT',
          (sys.argv[0], RYSE2D_CONSOLE_VERSION)))
    print(MultiLanguage.get_string('RYSE_HELP_AVAILABLE_CMD'))
    parse = Ryse2dIniParser()
    ...
    print(MultiLanguage.get_string('RYSE_HELP_AVAILABLE_ARGS_FMT',
                                   MultiLanguage.get_available_langs()))
    print(MultiLanguage.get_string('RYSE_HELP_EXAMPLE'))

def show_version():
    print(RYSE_ENGINE_VERSION)
    print("Ryse Console %s" % RYSE2D_CONSOLE_VERSION)
...
#        Logging.info(MultiLanguage.get_string('RYSE_INFO_RUNNING_PLUGIN_FMT', plugin.__class__.plugin_name()))
...
def _check_python_version():
    ...
    if not ret:
        print(MultiLanguage.get_string('RYSE_PYTHON_VERSION_TIP_FMT') % (major_ver, minor_ver))
...
if __name__ == "__main__":
    ...
    if language_arg in sys.argv:
        ...
        if idx == (len(sys.argv) - 1):
            Logging.error(MultiLanguage.get_string('RYSE_ERROR_OL_NO_VALUE'))
    ...
    if agreement_arg in sys.argv:
        ...
        if idx == (len(sys.argv) - 1):
            Logging.error(MultiLanguage.get_string('RYSE_ERROR_AGREEMENT_NO_VALUE'))
            sys.exit(CCPluginError.ERROR_WRONG_ARGS)
    ...
    RYSE_ENGINE_VERSION = utils.get_engine_version(engine_path)
    STAT_VERSION = RYSE_ENGINE_VERSION
    ver_pattern = r"ryse2d-x-(.*)"
    match = re.match(ver_pattern, RYSE_ENGINE_VERSION)
    if match:
        STAT_VERSION = match.group(1)

    DataStatistic.show_stat_agreement(skip_agree_value)
    DataStatistic.stat_event('ryse', 'start', 'invoked')
    ...
    try:
        ...
        if command in plugins:
            DataStatistic.stat_event('ryse', 'running_command', command)
            ...
            if len(sys.argv) > 2:
                ...
                if command in plugins:
                    ...
                    raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_CMD_NOT_FOUND_FMT',
                                                                 ' '.join(sys.argv[1:])),
                                        CCPluginError.ERROR_CMD_NOT_FOUND)
            else:
                raise CCPluginError(MultiLanguage.get_string('RYSE_ERROR_CMD_NOT_FOUND_FMT', command),
                                    CCPluginError.ERROR_CMD_NOT_FOUND)

    except Exception as e:
        # FIXME don't know how to handle this. Can't catch ryse2d.CCPluginError
